...
export function AssistenteChat() {
  ...
  function enviar() {
    ...
    iniciar(async () => {
      const r = await perguntarAssistente(pergunta, historico);
      setEnviando(false);
      if (!r.ok) {
        setErro(r.mensagem);
        return;
    ...
    <>
      ...
      <AnimatePresence>
        {aberto && (
          ...
          >
            <div className="flex items-center justify-between border-b border-[var(--rule)] bg-blue px-4 py-3">
            ...
            <div className="flex-1 space-y-3 overflow-y-auto p-4">
              {mensagens.length === 0 && (
                <p className="text-sm text-ink-muted">
                  ...
                  Eu só converso, não executo nenhuma operação.
                </p>
              )}
              {mensagens.map((m, i) => (
                <div
